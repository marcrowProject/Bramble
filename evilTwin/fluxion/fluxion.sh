...
if [ "$FLUX_AUTO" =  "1" ];then
		...
		############################# adding for bramble ###############################
		list_language="English German Romanian Turkish Spanish Chinese Italian Czech Greek French Slovenian"
		nb_Language=11
		tab_Language=($list_language)
		ans="n"
		echo ${tab_Language[0]}
		i=0
		while true; do
			clear
			echo -e ""$red"["$yellow"2"$red"]"$transparent" Select your language"
			echo -e $green"--"${tab_Language[i]}"--"$transparent

			tmp1=`expr $i + 1`
			tmp1=`expr $tmp1 % $nb_Language`
			tmp2=`expr $tmp1 + 5`
			for j in `seq $tmp1 $tmp2`;
			do
				echo ${tab_Language[`expr $j % $nb_Language`]}
			done
			echo "         |"
			echo "         V"
...
function selection {
	...
	##############################Adding for bramble##########################################
	j=0
	list_essid=$(echo $list_essid | sed "s/ESSID //g")
	tab_power=($list_power)
	...
	while true; do
		...
		echo "         |"
		echo "         V"
		echo -e ""$blue "("$red"*"$white") $selection_1"$transparent""
		read ans
		if [ $ans = "y" ]; then
			clear
			echo "you select : "${tab_essid[j]}" "$j
			break
		else
			j=`expr $j + 1`
			j=`expr $j % $i`
...
function handshakelocation {
        ...
        if [ "$handshakeloc" = "" ]; then
                ...
                echo -e "Press ${yellow} y or n $transparent to skip"
                echo
                echo -ne "Path: "
		...
		####################this line is modify for bramble  -v ###################
                if [ "$handshakeloc" = "y" ] || [ "$handshakeloc" = "n" ] || [ "$handshakeloc" = "" ]; then
                        deauthforce
                else
		...
		i=1
			...
			yn=1
		 	while true; do
		                ...
		                echo "                                       "
				if [ $i -eq 1 ]; then
					echo -e "      "$red"["$yellow"1"$red"]"$green"-- pyrit --" $transparent
				else
					echo -e ""$red"["$yellow"1"$red"]"$grey" pyrit" $transparent
				fi
				if [ $i -eq 2 ]; then
					echo -e "      "$red"["$yellow"2"$red"]"$green"-- $deauthforce_option_1 --"
				else
...
######################################Custom for bramble#####################################################
		i=1
                while true; do
                        ...
                        echo "                                       "
			if [ $i -eq 1 ]; then
				echo -e "      "$red"["$yellow"1"$red"]"$green"-- Deauth all --"$transparent
			else
	                        echo -e ""$red"["$yellow"1"$red"]"$grey" Deauth all"$transparent
			fi
			if [ $i -eq 2 ]; then
				echo -e "      "$red"["$yellow"2"$red"]"$green"-- Deauth all [mdk3] --"
			else
                        	echo -e ""$red"["$yellow"2"$red"]"$transparent" Deauth all [mdk3]"
			fi
			if [ $i -eq 3 ]; then
				echo -e "      "$red"["$yellow"3"$red"]"$green"-- Deauth target --"
			else
		...
		i=1
                while true; do
                      ...
                      #  top
                        ...
                        echo
                  			if [ $i -eq 1 ]; then
                  				echo -e "      "$red"["$yellow"1"$red"]"$green"-- $deauthMENU_option_1 --"
                  			else
                  		                echo -e ""$red"["$yellow"1"$red"]"$grey" $deauthMENU_option_1"
                  			fi
                  			if [ $i -eq 2 ]; then
                  				echo -e "      "$red"["$yellow"2"$red"]"$green"-- $general_back --"
                  			else
                  		                echo -e ""$red"["$yellow"2"$red"]"$transparent" $general_back "
                  			fi
                  			if [ $i -eq 3 ]; then
                  				echo -e "      "$red"["$yellow"3"$red"]"$green" Select another network"
                  			else
                  		                echo -e ""$red"["$yellow"3"$red"]"$transparent" Select another network"
                  			fi
                  			if [ $i -eq 4 ]; then
                  				echo -e "      "$red"["$yellow"4"$red"]"$green" Exit"
                  				i=0
                  			else
...
############################################# custom for bramble ############################################
                        ...
                        while true;do
                              ...
                              echo "                                       "
                              if [ $i -eq 1 ]; then
                                    echo -e "      "$red"["$yellow"1"$red"]"$green"-- Create a SSL certificate --"
                              else
                                    echo -e ""$red"["$yellow"1"$red"]"$grey" Create a SSL certificate"
                              fi
                              if [ $i -eq 2 ]; then
                                    echo -e "      "$red"["$yellow"2"$red"]"$green"-- Search for SSL certificate --" # hop to certssl check again
                              else
                                    echo -e ""$red"["$yellow"2"$red"]"$transparent" Search for SSL certificate" # hop to certssl check again
                              fi
                              if [ $i -eq 3 ]; then
                                    echo -e "      "$red"["$yellow"3"$red"]"$green"-- Exit --" $transparent
                                    i=0
                              else
...
############################################ Custom for Bramble #####################################
                ...
                while true; do
                        ...
                        echo
                        if [ $i -eq 1 ]; then
                              echo -e "      "$red"["$yellow"1"$red"]"$green"-- Web Interface --"
                        else
                              echo -e ""$red"["$yellow"1"$red"]"$grey" Web Interface"
                        fi
                        if [ $i -eq 2 ]; then
                              echo -e "      "$red"["$yellow"2"$red"]"$green"-- Exit --"$transparent""
                              i=0
                        else
...
function ConnectionRESET {
        ...
        if [ "$FLUX_AUTO" = "1" ];then
                ...
                while true; do
                        ...
                        echo
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent"  English     [ENG]  (NEUTRA)";list_website=$list_website" English";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent"  German      [GER]  (NEUTRA)";list_website=$list_website" German";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent"  Russian     [RUS]  (NEUTRA)";list_website=$list_website" Russian";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent"  Italian     [IT]   (NEUTRA)";list_website=$list_website" Italian";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent"  Spanish     [ESP]  (NEUTRA)";list_website=$list_website" Spanish";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent"  Portuguese  [POR]  (NEUTRA)";list_website=$list_website" Portuguese";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent"  Chinese     [CN]   (NEUTRA)";list_website=$list_website" Chinese";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent"  French      [FR]   (NEUTRA)";list_website=$list_website" French";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent"  Turkish     [TR]   (NEUTRA)";list_website=$list_website" Turkish";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Romanian    [RO]   (NEUTRA)";list_website=$list_website" Romanian";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Hungarian   [HU]   (NEUTRA)";list_website=$list_website" Hungarian";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Arabic      [ARA]  (NEUTRA)";list_website=$list_website" Arabic";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Greek       [GR]   (NEUTRA)";list_website=$list_website" Greek";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Czech       [CZ]   (NEUTRA)";list_website=$list_website" Czech";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Norwegian   [NO]   (NEUTRA)";list_website=$list_website" Norvegian";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Bulgarian   [BG]   (NEUTRA)";list_website=$list_website" Bulgarian";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Serbian     [SRB]  (NEUTRA)";list_website=$list_website" Serbian";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Polish      [PL]   (NEUTRA)";list_website=$list_website" Polish";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Indonesian  [ID]   (NEUTRA)";list_website=$list_website" Indonesian";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Dutch       [NL]   (NEUTRA)";list_website=$list_website" Dutch";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Danish      [DAN]  (NEUTRA)";list_website=$list_website" Danish";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Hebrew      [HE]   (NEUTRA)";list_website=$list_website" Hebrew";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Thai        [TH]   (NEUTRA)";list_website=$list_website" Thai";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Portuguese  [BR]   (NEUTRA)";list_website=$list_website" Portuguese";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Slovenian   [SVN]  (NEUTRA)";list_website=$list_website" Slovenian";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Belkin      [ENG]";list_website=$list_website" belkin_eng";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Netgear     [ENG]";list_website=$list_website" netgear_eng";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Huawei      [ENG]";list_website=$list_website" huawei_eng";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Verizon     [ENG]";list_website=$list_website" verizon_eng";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Netgear     [ESP]";list_website=$list_website" netgear_esp";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Arris       [ESP]";list_website=$list_website" arris_esp";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Vodafone    [ESP]";list_website=$list_website" vodafone_esp";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" TP-Link     [ENG]";list_website=$list_website" tp-link_eng";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Ziggo       [NL]";list_website=$list_website" ziggo_nl";n=` expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" KPN         [NL]";list_website=$list_website" kpn_nl";n=` expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Ziggo2016   [NL]";list_website=$list_website" Ziggo2016_nl";n=` expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" FRITZBOX_DE [DE] ";list_website=$list_website" fritzbox_de";n=` expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" FRITZBOX_ENG[ENG] ";list_website=$list_website" fritzbox_eng";n=` expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" GENEXIS_DE  [DE] ";list_website=$list_website" genenix_de";n=` expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Login-Netgear[Login-Netgear] ";list_website=$list_website" login-Netgear";n=` expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Login-Xfinity[Login-Xfinity] ";list_website=$list_website" login-Xfinity";n=` expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Telekom ";list_website=$list_website" Telekom";n=` expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Google";list_website=$list_website" Google";n=` expr $n + 1`
      echo -e "      "$red"["$yellow"$n"$red"]"$transparent" MOVISTAR     [ESP]";list_website=$list_website" movistar_esp";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Custom_1";list_website=$list_website" Custom_1";n=` expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Custom_2";list_website=$list_website" Custom_2";n=` expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Custom_3";list_website=$list_website" Custom_3";n=` expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent"\e[1;31m $general_back"$transparent"";list_website=$list_website" Exit";
                        echo
                        echo -n "#? "
                        ...
                        j=45
                        webconf=45
                        tab_website=($list_website)
                        while true; do
                                conditional_clear
                                k=`expr $j + 1`
                                echo -e "   "$red"["$yellow"$k"$red"]"$green"-- ${tab_website[j]} --"
                                tmp=`expr $k + 5`
                                for nsite in `seq $k $tmp`
                                do
                                  echo -e ""$red"["$yellow"$nsite"$red"]"$transparent" ${tab_website[$(((nsite)%(n)))]}"
                                done
                                j=$(((j+1)%(n)))
                                echo "      |"
                                echo "      V"
...
function Custom1 {
    mkdir $DUMP_PATH/data &>$flux_output_device
    cp -r $WORK_DIR/sites/custom/CUSTOM1/* $DUMP_PATH/data
}
function Custom2 {
      mkdir $DUMP_PATH/data &>$flux_output_device
      cp -r $WORK_DIR/sites/custom/CUSTOM2/* $DUMP_PATH/data
}
function Custom3 {
        mkdir $DUMP_PATH/data &>$flux_output_device
        cp -r $WORK_DIR/sites/custom/CUSTOM3/* $DUMP_PATH/data
  }
